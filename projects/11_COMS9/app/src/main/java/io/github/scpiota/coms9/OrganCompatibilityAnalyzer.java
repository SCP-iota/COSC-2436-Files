...
import io.github.scpiota.coms9.queue.WaitingList;
import io.github.scpiota.coms9.queue.WaitingListNode;
...
public class OrganCompatibilityAnalyzer {
    ...
    Patient findCompatiblePatient(CyberneticOrgan organ, WaitingList waitingList) {
        Stream.Builder<WaitingListNode> builder = Stream.builder();
        var curr = waitingList.getHead();
        ...
        var compatiblePatients = builder.build()
            .filter(node -> isCompatible(organ, node.getPatient()))
            .sorted((n1, n2) -> {
        ...
        if(compatiblePatients.size() > 0) {
            return compatiblePatients.get(0);
        } else {
            return null;
        }
    }
